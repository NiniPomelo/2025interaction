...
// 修改自 week10_6_arduino_serial_joystick_x_y_to_processing
void setup() {
  Serial.begin(9600); 
...
void loop() { 
  ...
  else x = (x-x0)/4.4 + 128; // 有大的數值,就減掉中間值,在除4.4,在加128 
  if ( abs(y-y0) < 15 ) y = 128; // 數值變化太小, 直接放中間值128
  else x = (y-y0)/4.4 + 128; // 有大的數值,就減掉中間值,在除4.4,在加128
